...
public class Teleop {
    ...
    private void p3Loop(GenericHID p) {
        // Human Pickup Station
        if (p.getRawButtonPressed(10) && !swerve.isOverLimit() && !arm.isGoingHome() && arm.isOnTarget()
                && arm.isSafe()) {
            arm.configSmartMotionConstraints(
                    ArmConstants.LOWER_MAX_VEL * 1.5,
                    ArmConstants.LOWER_MAX_ACCEL * 1.5,
        ...
        // High Grid
        else if (p.getRawButtonPressed(14) && !swerve.isOverLimit() && !arm.isGoingHome() && arm.isOnTarget()
                && arm.isSafe()) {
        ...
        // Medium Grid
        else if (p.getRawButtonPressed(15) && !swerve.isOverLimit() && !arm.isGoingHome() && arm.isOnTarget()
                && arm.isSafe()) {
        ...
        // Floor Intake
        else if (p.getRawButtonPressed(16) && !swerve.isOverLimit() && !arm.isGoingHome() && arm.isOnTarget()
                && arm.isSafe()) {
            arm.moveTwoPronged(-ArmConstants.INTER_FLOOR_INTAKE[0], ArmConstants.INTER_FLOOR_INTAKE[1], 155,
                    -ArmConstants.FLOOR_INTAKE[0], ArmConstants.FLOOR_INTAKE[1], 155);
        }
        // Go Home
